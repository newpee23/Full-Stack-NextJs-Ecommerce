    ...
    <div onClick={() => cartStore.toggleCart()} className="fixed w-full h-screen top-0 left-0 bg-black/25 z-50">
      <div onClick={(e) => e.stopPropagation()} className="bg-white absolute right-0 top-0 md:w-2/5 w-3/4 h-screen p-12">
        ...
        {cartStore.onCheckout === "cart" && (
          <>
            <button onClick={() => cartStore.toggleCart()} className="text-sm font-bold">
              Back to store
            </button>
            {cartStore.cart.map((product) => (
              <div key={product.id} className="flex md:flex-row flex-col py-4 gap-4 items-center border-b-grey-600 border-b-2 justify-between mb-5">
